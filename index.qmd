...
- Average of each unique subgroup of `married` and `race`:
    ...
    produce all of `margins married`, `margins race`, and `margins
    married#race`.
...
- Average effect of `marriage` within levels of `race`:
    ...
    Note the use of `@` instead of `#` when dealing with the `constrast()`
    option.
...
|:-:|:-:|
| White vs Black |       -0.069 |
| White vs Other |       -0.686 |
| Black vs Other | ??? |
:::
...
>     (black_other: exp(_b[2.race] - _b[3.race]))
 ...
 white_black: exp(_b[2.race])
 white_other: exp(_b[3.race])
 black_other: exp(_b[2.race] - _b[3.race])

------------------------------------------------------------------------------
       union | Coefficient  Std. err.      z    P>|z|     [95% conf. interval]
-------------+----------------------------------------------------------------
 white_black |   1.501429   .1760094     8.53   0.000     1.156456    1.846401
 white_other |   1.740066   .7619876     2.28   0.022     .2465981    3.233534
 black_other |   .8628571   .3829566     2.25   0.024     .1122759    1.613438
------------------------------------------------------------------------------
...
- Higher-order interactions work as expected:
  ...
  - `margin a#c, at(b = (3 5))` (if `b` is continuous only)
